...
log("info", "Throttlade inpage script loaded");
...
window.fetch = async (input: RequestInfo | URL, init?: RequestInit) => {
  log("debug", "Throttlade fetch called", { input, init });
  const req =
    typeof input === "string"
  ...
  if (rule) {
    log("info", "Throttlade fetch intercepted", { url: req.url, method: req.method, delayMs: rule.delayMs });
...
XMLHttpRequest.prototype.open = function (method: string, url: string, ...rest: any[]) {
  log("debug", "Throttlade XMLHttpRequest intercepted", { method, url });
  log("debug", "Throttlade rules", rules);
...
XMLHttpRequest.prototype.send = async function (...args: any[]) {
  ...
  log("debug", "Throttlade XMLHttpRequest send called", { args, ctx });
  ...
  log("debug", "Throttlade XMLHttpRequest send rules?", { rules, ctx });
  log("info", "Throttlade XMLHttpRequest rule match", { rule, ctx });
  if (rule) {
    log("info", "Throttlade XHR delaying", { url: ctx!.url, method: ctx!.method, delayMs: rule.delayMs });
    // Old timeout-based delay (commented out during strategy integration):
    // await delay(rule.delayMs);
...
window.addEventListener("message", (ev: MessageEvent) => {
  ...
  if (d && d.__THROTTLR__ && d.type === "STATE") {
    if (Array.isArray(d.rules)) {
      rules = d.rules;
      log("info", "[Throttlade][inpage] rules updated", rules);
    }
    if (typeof d.enabled === "boolean") {
      enabled = d.enabled;
      log("info", "[Throttlade][inpage] enabled updated", { enabled });
    }
  }
});
